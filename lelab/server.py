...
from .chat import (
    ...
    ChatRequest,
    handle_chat,
    handle_get_chat_config,
    handle_set_chat_config,
)
from .dataset_detail import (
    DatasetEpisodesRequest,
...
@app.get("/chat/config")
def get_chat_config():
    return handle_get_chat_config()


@app.post("/chat/config")
def set_chat_config(update: ChatConfigUpdate):
    return handle_set_chat_config(update)
...
def datasets_list():
    ...
    Named `/list-datasets` (not `/datasets`) so it doesn't shadow the frontend's
    `/datasets` page route when the SPA is served from this same origin.
    """
...
def delete_dataset(request: DeleteDatasetRequest):
    """Remove a recorded dataset from local disk (and optionally from the Hub)."""
    return handle_delete_dataset(request)


@app.post("/dataset-sync-status")
def dataset_sync_status(request: DatasetInfoRequest):
    """Report whether a dataset's local files match the Hub copy."""
    return handle_dataset_sync_status(request)


@app.post("/dataset-episodes")
def dataset_episodes(request: DatasetEpisodesRequest):
    """List a local dataset's episodes (frame counts, durations, cameras)."""
    return handle_list_episodes(request)


@app.post("/import-dataset")
def import_dataset(request: DatasetInfoRequest):
    """Download a Hub dataset into the local cache so it can be viewed in-app."""
    return handle_import_dataset(request)


@app.post("/delete-episode")
def delete_episode(request: DeleteEpisodeRequest):
    """Delete one local dataset episode by rewriting data/video/metadata files."""
    return handle_delete_episode(request)


@app.get("/dataset-episode-video")
def dataset_episode_video(repo_id: str, episode: int, camera: str, request: Request):
    """Stream one episode's MP4 for a camera, honoring HTTP Range (for <video> seek)."""
    try:
        path = resolve_episode_video(repo_id, episode, camera)
...
def get_runners_hardware():
    """Return HF Jobs flavor catalog + auth state for the training target picker.
...
def _windows_cameras() -> list[dict[str, Any]]:
    ...
    if not names:
        logger.warning("pygrabber returned no DirectShow cameras; using generic camera probe")
        import cv2
