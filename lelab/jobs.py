...
def _training_subprocess_env() -> dict[str, str]:
    """Environment for local training subprocesses.

    The sitecustomize directory carries Windows-only compatibility patches for
    LeRobot training. Prepending it keeps the parent API process untouched while
    letting the child import the patch before `lerobot.scripts.lerobot_train`.
    """
    child_env = os.environ.copy()
    child_env["PYTHONUNBUFFERED"] = "1"

    shim_dir = Path(__file__).parent / "_train_sitecustomize"
...
class LocalJobRunner:
    ...
    def start(
        ...
        child_env = _training_subprocess_env()
