...
const InferenceModal: React.FC<Props> = ({
  ...
  // across cv2 index drift); fall back to the saved camera_index. If there is
  // exactly one expected camera and one detected camera, bind it even when the
  // browser preview metadata is unavailable.
  useEffect(() => {
    ...
    setCameraBindings((prev) => {
      let changed = false;
      const next = { ...prev };
      const policyNames = Object.keys(policyConfig.image_features);
      for (const policyName of policyNames) {
        ...
        const live = robotCam
          ...
          : policyNames.length === 1 && availableCameras.length === 1
            ? availableCameras[0]
            : undefined;
        if (live) {
          ...
          <div className="space-y-4">
            ...
            {!robot ? (
              <Alert className="bg-amber-900/40 border-amber-700 text-amber-100">
                ...
                <AlertDescription>
                  Select and configure a robot on the Robot page first.
                </AlertDescription>
              </Alert>
            ) : !robot.is_clean ? (
              <Alert className="bg-amber-900/40 border-amber-700 text-amber-100">
