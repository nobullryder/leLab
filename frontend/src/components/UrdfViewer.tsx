...
const UrdfViewer: React.FC = () => {
  ...
  return (
    <div className="relative h-full w-full bg-[radial-gradient(120%_120%_at_50%_0%,#11161b,#090c0e)] transition-all duration-300 ease-in-out">
      <div ref={containerRef} className="h-full w-full" />

      {/* Joint highlight indicator */}
      {highlightedJoint && (
        <div className="pill absolute bottom-4 right-4 z-10 normal-case">
          <span className="text-[var(--ink-faint)]">joint</span>
          <span className="text-foreground">{highlightedJoint}</span>
        </div>
      )}
      ...
      {isDefaultModel && (
        <div className="absolute right-4 top-4 z-10">
          <div className={isWebSocketConnected ? "pill pill-live" : "pill pill-stop"}>
            <span className={isWebSocketConnected ? "dot dot-live" : "dot dot-stop"} />
            {isWebSocketConnected ? "Live data" : "Disconnected"}
          </div>
        </div>
      )}
    </div>
