...
lelab --dev        # Vite dev (:8080) + uvicorn --reload (:8000), opens browser to :8080
lelab --rebuild    # rebuilds frontend/dist, then starts production mode
lelab --no-open    # starts without opening a browser
```

When `frontend/**` (excluding `frontend/dist/**`) changes on `main`, [`.github/workflows/build_frontend.yml`](.github/workflows/build_frontend.yml) rebuilds `frontend/dist/` and fails if the committed bundle is stale. It does not commit or push generated assets. Run `cd frontend && npm run build` before committing frontend changes that should update the production bundle. `lelab --dev` serves directly from Vite, skips `npm install` when `frontend/node_modules` already exists, and does not require a rebuild.
