...
def test_training_subprocess_env_prepends_sitecustomize(monkeypatch: pytest.MonkeyPatch) -> None:
    from lelab.jobs import _training_subprocess_env

    monkeypatch.setenv("PYTHONPATH", "existing")

    env = _training_subprocess_env()
    pythonpath = env["PYTHONPATH"].split(__import__("os").pathsep)

    assert Path(pythonpath[0]).name == "_train_sitecustomize"
...
def test_list_imported_local_checkpoints_tree(tmp_path) -> None:
    ...
    assert out[0].source == "local"
    assert out[0].ref.endswith(str(Path("checkpoints") / "000010" / "pretrained_model"))
